...
windows_batch "unzip_nova" do
  ...
  #{node[:sevenzip][:command]} x #{node[:openstack][:location]}\\dist\\dist\\#{node[:openstack][:nova][:name]}-#{node[:openstack][:nova][:version]}.tar -o#{node[:openstack][:location]} -r -y
...
windows_batch "unzip_quantum" do
  ...
  #{node[:sevenzip][:command]} x #{node[:openstack][:location]}\\dist\\dist\\#{node[:openstack][:quantum][:name]}-#{node[:openstack][:quantum][:version]}.tar -o#{node[:openstack][:location]} -r -y
  rmdir /S /Q #{node[:openstack][:location]}\\dist
